...
class Kucoin(object):
    ...
    def __init__(self, key='', secret='', passphrase='', is_sandbox=False, url='', is_v1api=False):
        ...
        try:
            ...
            self.version = 'v' + pkg_resources.get_distribution("kucoin-python").version
    ...
    def _request(self, method, uri, timeout=5, auth=True, params=None):
        ...
        headers["User-Agent"] = "kucoin-python-sdk/"+self.version
    ...
    # define the api port
    def send_public_request(self, uri_path, params = {}, method = 'GET'):
        return self._request(method, uri_path, params=params)
    
    def send_signed_request(self, uri_path, params = {}, method = 'GET', key = '', secret = '', passphrase = ''):
        
        # null api keys
        if not ((self.key and self.secret and self.passphrase) or (key and secret and passphrase)):
            print('Please set key, secret and passphrase')
            return 
        
        # input the api keys
        elif (key and secret and passphrase):
            self.key = key
            self.secret = secret
            self.passphrase = passphrase
